...
target=[2.88998 3.04461 2.95448 3.02059 2.85928 3.08949 2.81174 3.13774];
target=[target; 2.77201 3.14102 2.9704 3.01266 2.91304 3.05207 2.84834 3.11685];
target=[target; 2.92383 3.02218 2.85284 3.11454 2.79458 3.14516 2.89292 3.08163];
target=[target; 2.95872 2.98742 2.7962 3.154 2.93938 3.02713 2.83165 3.12803];
target=[target; 2.95339 2.97866 2.924 3.04072 2.8148 3.11631 2.8738 3.08318];
target=target*10^3;
...
for k=1:permutations
    ...
    for m=1:4
        for n=1:4
            for o=1:4
                %[B(k,:,:),
                %residual(k,:)]=ExactBfield(targetNew,[B0init(:,:);
                %thetainit(:,:); phiinit(:,:)], Dinit);
            end
        end
    end
...
%Bfinal(3,2)=Bfinal(3,2)+pi; %first index is iteration; second is [mag theta phi]
...
function [B,Del] = LinearGuess(energy)
...
for j=1:size(Bguess,2) % four different ways to choose three spectra from four
    [Btemp(j,:),resnorm(j)] = lsqnonlin(@Projections,[Bguess(:,j), Dinit],[],[],opts1);
end
...
% these four indices correspond to each NV axis
